...
class Message:
    ...
    def __init__(self, message_id: int = 0, sender_name: str = "", sender_id: int = 0, chat_id: int = 0, text: str = "", reply_to_message_id: int = 0) -> None:
...
class App:
    ...
    def __init__(self) -> None:
        ...
        self.DB.create_function('regexp', 2, lambda x,
                                y: 1 if re.search(x, y) else 0)
        token = config.token
        if os.getenv("TOKEN"):
            token = os.getenv("TOKEN")
        self.baseURL = "https://api.telegram.org/bot{}/".format(token)
        self.DB.execute("CREATE TABLE IF NOT EXISTS messages (    MessageID        integer,\
    SenderName       text,\
    SenderID         int,\
    ChatID           integer,\
    Message          text,\
    ...
    def getPreviousMessage(self, message: Message, reg: str) -> Message:
        ...
        data = cur.fetchone()
        mes = Message(message_id=data[0], sender_name=data[1], sender_id=data[2],
                      chat_id=data[3], text=data[4], reply_to_message_id=data[5])
        return mes
    ...
    def getClosedThread(self) -> str:
        ...
        closed_thread_replies = [
            "rekt", "*This thread has been archived at RebeccaBlackTech*"]
    ...
    def getSedCommand(self, message: Message) -> str:
        ...
        if reply_message is not None:
            try:
                reply = re.sub(
                    old, new, reply_message.messageText, max_replace)
    ...
    def insertMessage(self, message: Message) -> None:
        ...
        self.DB.execute(query, (message.messageID, message.senderName,
                        message.senderID, message.chatID, message.messageText, message.replyID))
        self.DB.commit()
    ...
    def startBot(self) -> None:
        ...
        while True:
            ...
            for item in updates:
                ...
                message = Message(message_id, sender_name,
                                  sender_id, chat_id, text, reply_to_message_id)
                ...
                if sent_message is not None and "result" in sent_message:
                    ...
                    message = Message(sent_message_id, sent_message_sender_name,
                                      sent_message_sender_id, message.chatID, reply, None)
                    self.insertMessage(message)
